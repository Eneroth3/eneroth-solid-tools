...
class Solids
  ...
  #
  # group_or_component - The group or component to test.#NOTE: VAR NAMEAS: all group_or_compknent solid instead. or entity?
  #
  ...
  def self.inside_solid?(point, group_or_component, return_value_when_on_face = true)
    ...
    vector = Geom::Vector3d.new 234, 1343, 345#NOTE: using random vector makes code work in most cases. why didn't x axis work? did ray hit between 2 faces counting them as 2?
vector.reverse!
    line = [point, vector]
    intersections = []
    ents = entities_from_group_or_componet group_or_component
...
#puts "classify point: #{clasify_point}"      
      return return_value_when_on_face if clasify_point == Sketchup::Face::PointInside#NOTE: doesn't return here when on face :O
...
#p intersections.length
  ...
  def self.union(original, to_add)#NOTE: optional wrap-in-operator argument.
    ...
    #NOTE: do this with the least amount of code possible. perhaps some sort of loop since same code runs on both groups. could also be separate method.
    to_delete = []
    original_ents.each do |e|
      next unless e.is_a? Sketchup::Edge
      midpoint = Geom.linear_combination 0.5, e.start.position, 0.5, e.end.position
      midpoint.transform! original.transformation
      next unless inside_solid? midpoint, to_add, false
      #to_delete << e
e.material = Sketchup::Color.new "Red"
    end
    original_ents.erase_entities to_delete
