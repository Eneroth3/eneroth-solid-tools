...
class Solids
  ...
  def self.union(original, to_add, wrap_in_operator = true)
    ...
    #NOTE: SOLIDS: remove all faces in both solids with a corresponding face in the other with opposite orientation.
  ...
  def self.subtract(original, to_subtract, wrap_in_operator = true)
    ...
    #Check if both groups/components are solid.
    return if !is_solid?(original) || !is_solid?(to_subtract)#NOTE: to_subtract should be made unique, without littering the component browser.
    ...
    #NOTE: SOLIDS: remove all faces in both solids with a corresponding face in the other with same orientation.
    ...
    #   Faces that where in the same plane in the different solids may have been#NOTE: REMOVE: is this comment necessary? will those faces be removed before this?
  ...
  def self.double_intersect(ent0, ent1)
    ...
    #Double intersect so intersection edges appear in both contexts.
    #NOTE: CONTEXT: DOCUMENT (or fix): groups/components must be in the same drawing context for these transformations to work.
  ...
  def self.point_in_face(face)
    ...
    face.vertices.each_with_index do |v, i|
      p = v.position
      p_before = face.vertices[i-1].position#Use 3 points to allow for triangles.
      p_2nd_before = face.vertices[i-2].position
      p_temp = Geom.linear_combination 0.5, p, 0.5, p_2nd_before
      p_check = Geom.linear_combination 0.5, p_temp, 0.5, p_before
      return p_check if face.classify_point(p_check) == Sketchup::Face::PointInside
    end
  ...
  def self.find_faces(to_search_in, reference, inside, on_surface)
    ...
    to_remove_in_ents.select do |f|
      ...
      next if inside != inside_solid?(point, reference, inside == on_surface)
      true
    end
  ...
  #                    false to only return those oriented the oposite
  ...
  def self.find_corresponding_faces(ent0, ent1, same_orientation)
    ...
    ents0 = ent0.entities#NOTE: use separate method that works on components too
    ents1 = ent1.entities
    ...
    ents0.each do |f0|
      ...
      ents1.each do |f1|
        ...
        unless same_orientation.nil?
          next if normal0.samedirection?(normal1) != same_orientation#NOTE: this check is based on face orientation. Faces might be faulty oriented.
        end
        faces << f0
        faces << f1
      end
  ...
  # (In subtract an edge can temporarily bind 3 faces because faces that has
  # exact corresponding faces on other solid are kept). #NOTE: REMOVE STUFF: not if faces on surface are removed too. then this can check for exactly 2 and strange comment where it's called in subtract can be removed.
  ...
  def self.move_into(destination, to_move)#NOTE: CONTEXT: perhaps requires solids to be in same entities.
