...
class Solids
  ...
  def self.union(original, to_add)#NOTE: optional wrap-in-operator argument.
    ...
    #NOTE: do this with the least amount of code possible. separate method. ent1 + ent2, remove inside/outside? on surface?
    to_delete = []
...
p to_delete
    ...
    to_delete = []
    to_add_ents.each do |e|
      next unless e.is_a? Sketchup::Edge
      midpoint = Geom.linear_combination 0.5, e.start.position, 0.5, e.end.position
      midpoint.transform! to_add.transformation
      next unless inside_solid? midpoint, original, false
      to_delete << e
    end
p to_delete
    to_add_ents.erase_entities to_delete
