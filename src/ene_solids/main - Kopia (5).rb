...
class Solids
  ...
  def self.union(original, to_add, wrap_in_operator = true)
    ...
    #...
    #Don't remove where the material or layer differs.
  ...
  #
  # Returns nothing.
  def self.subtract(original, to_subtract, wrap_in_operator = true)#NOTE: mostly same as union. Add separate do_solid/do_bool private method called with all differences in code as attributes? must also support intersect and similar that creates new groups.
...
puts "Subtract code not yet written"
  ...
  self.double_intersect(ent0, ent1)

    ent0_ents = entities_from_group_or_componet ent0
  ...
  def self.find_ents_to_remove(to_remove_in, refernce, inside, on_surface)
    ...
    to_remove_in_ents.each do |e|
      ...
      next unless inside_solid? midpoint, refernce, false#NOTE: UNDER CONSTRUCTION: adapt to remove inside, outside, on surface
      to_erase << e
    end

    to_erase#NOTE: BETTER CODE: use consistent terms. delete, remove, erase
    #NOTE: also check faces. especially faces inside other faces could be kept with this code.

  end

  def self.move_into(destination, to_move)
    ...
    trans = to_move.transformation*(destination.transformation.inverse)#NOTE: only works when axes are parallel. transform transformation similar to what is done in ene_railroad to change group transformation while keeping geometry in same place.
