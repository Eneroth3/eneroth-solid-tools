...
Sketchup.require File.join(PLUGIN_DIR, "vendor", "cmty-lib", "geom")
...
module SolidOperations
  ...
  def self.solid?(container)
    return false unless LEntity.instance?(container)

    LEntity.definition(container).entities.grep(Sketchup::Edge).all? { |e| e.faces.size.even? }
  end
  ...
  def self.within?(point, container, on_boundary = true, verify_solid = true)
    ...
    LEntity.definition(container).entities.grep(Sketchup::Face) do |face|
      return on_boundary if LFace.includes_point?(face, point)
      ...
      next unless LFace.includes_point?(face, intersection)
    ...
    intersections = LGeom.remove_duplicates(intersections)
  ...
  def self.union(target, modifier)
    ...
    target_ents = LEntity.definition(target).entities
    modifier_ents = LEntity.definition(modifier).entities
  ...
  def self.add_intersection_edges(container1, container2)
    entities1 = LEntity.definition(container1).entities
    entities2 = LEntity.definition(container2).entities
  ...
  def self.find_faces(scope, reference, interior, on_surface)
    LEntity.definition(scope).entities.select do |f|
      next unless f.is_a?(Sketchup::Face)
      point = LFace.arbitrary_interior_point(f)
      next unless point
  ...
  def self.find_corresponding_faces(container1, container2, orientation)
    ...
    LEntity.definition(container1).entities.grep(Sketchup::Face) do |face1|
      normal1 = LGeom::LVector3d.transform_as_normal(face1.normal, container1.transformation)
      points1 = face1.vertices.map { |v| v.position.transform(container1.transformation) }
      LEntity.definition(container2).entities.grep(Sketchup::Face) do |face2|
        next unless face2.is_a?(Sketchup::Face)
        normal2 = LGeom::LVector3d.transform_as_normal(face2.normal, container2.transformation)
  ...
  def self.merge_into(destination, to_move, keep_original = false)
    ...
    entities = LEntity.definition(destination).entities
    temp = entities.add_instance(LEntity.definition(to_move), tr)
  ...
  def self.interior_hole_hack(edges)
    ...
    # recess/bump.
    entities.erase_entities(new_faces.select { |f| !LFace.wrapping_face(f) || f.edges.any? { |e| e.faces.size != 2 } })
